...
本手册对应提示词 13 的比赛 Demo。默认只使用三套合成家庭、SQLite、版本化本地规则、受控知识和 Mock LLM；不需要 API Key、真实银行接口、行情服务或互联网。页面顶部和正式报告持续显示“竞赛原型”与 Mock／人工复核状态。
...
等待 `backend` 健康后打开 <http://localhost:8080/demo>。首次启动自动执行 Alembic 迁移并在空库导入 A／B／C 三套合成家庭。默认端口冲突时可以显式改映射：
...
脚本只允许 localhost／回环地址，并检查健康、Mock 默认、种子、预热、主计算、信用卡边界、三家庭唯一配置、适当性拒绝、十阶段主 Demo、性能目标、数字孪生、严格八章报告、十项发布门禁、八项安全对抗、七项实验和五个页面。输出是 JSON 证据账本，任何关键失败返回非零状态。
...
| 数据集不完整 | `/api/v1/demo/manifest` | 管理员点击“加载 A／B／C”；必要时显式重置合成数据 |
