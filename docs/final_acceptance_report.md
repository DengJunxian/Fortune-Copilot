...
2. **方案因人而异**：A／B／C 调用同一规则得到三个唯一配置签名，`fixed_ratio_model=false`。
...
6. **离线现场可完整演示**：默认 SQLite + Mock LLM + Mock 银行适配器，无密钥、真实银行接口或外部网络也能完成十阶段主 Demo。
...
| ---: | --- | --- | --- | --- |
| 1 | 是否明确说明项目不是标准普尔四象限图的本地化复制？ | 已实现 | `README.md`；`docs/technical_whitepaper.md`；`/demo` 三家庭对照 | `backend/tests/test_demo_release.py`；22 项黑盒验收 |
...
| 21 | 是否生成美观且可解释的家庭财务报表可视化？ | 已实现 | `DESIGN.md`；`frontend/src/components/`；`output/playwright/stage9-client-balance-1366x768.png` | 34 项 Vitest；15 项 Playwright；无障碍与图表数据表回归 |
...
| 29 | 是否提供客户端、顾问端和风险端闭环？ | 已实现 | `/client`、`/advisor`、`/risk`；八状态工作流与同源报告 | `backend/tests/test_review_workflow.py`；15 项 Playwright |
| 30 | 是否支持离线 Mock 演示且不伪造工行接口或真实收益？ | 已实现 | `docker-compose.yml`；`backend/app/services/demo_release.py`；Mock 标识与边界文案 | 22 项黑盒验收；外部调用 0；三端浏览器外部请求 0 |
