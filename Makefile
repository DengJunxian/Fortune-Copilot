...
.PHONY: setup setup-backend setup-frontend dev backend-dev frontend-dev migrate seed reset-demo validate-public-data warmup acceptance final-acceptance competition-benchmark backup-demo restore-demo test test-backend test-frontend test-e2e lint typecheck build check security-audit up down clean
...
backend-dev:
	cd backend && PYTHONPATH=. ../$(VENV_PYTHON) -m uvicorn app.main:app --reload --host 0.0.0.0 --port 8000
...
competition-benchmark:
	$(VENV_PYTHON) scripts/run_competition_benchmark.py
...
lint:
	cd backend && PYTHONPATH=. ../$(RUFF) check app tests alembic
	npm --workspace frontend run lint

typecheck:
	cd backend && PYTHONPATH=. ../$(MYPY) app
