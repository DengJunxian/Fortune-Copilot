...
传统智能投顾通常从风险、目标与可投资资产进入配置。Fortune Copilot 的增量价值，是把家庭责任与资金资格放在配置之前：先回答“这笔钱能不能投”，再回答“应该怎么投”。它不是从产品开始的智能投顾，而是从家庭责任开始的智能投顾。

## 为什么需要它？

一个家庭的金融资产可能同时承担日常周转、失业应急、房贷、教育、保障和养老责任。把全部金融资产直接视为可投资本金，会把近期责任暴露在不必要的市场波动中。

Fortune Copilot 以中国家庭财富健康 CHFH 为总体框架，保持以下决策顺序：

```text
家庭事实 → 人生目标 → 家庭责任 → 财务安全
→ 长期可投资资本 ELTC → 家庭风险预算 → 资产配置 → 产品候选
```

关键金额、比率、风险预算、ELTC、配置和产品资格均由确定性算法或受控规则计算。AI 负责理解、追问和解释，不获得关键金融数字决策权。

## 核心创新

1. **家庭责任驱动**：决策主体是 Household，同时核对成员、收入、支出、资产、负债、住房、保障、教育、赡养、养老金与企业暴露。
2. **长期可投资资本 ELTC**：从可调度金融资源中依次扣除周转、应急、高息债务、保障、近期责任、已承诺目标和锁定资金。
3. **GRB 家庭风险预算**：Goal + Risk + Behavior 联合约束风险；真实行为只能保持或降低风险预算。
4. **持续财富管理**：收入、成员、房贷、目标、产品到期、市场冲击或行为变化后重新计算。
5. **可解释财富建议**：输入、规则、产品快照、选择理由、拒绝理由与决策证据都可追溯和回放。

## 一个家庭如何使用？

以上海双收入三口之家为例：

1. 用一句自然语言描述收入、房贷、孩子和教育目标，系统先生成待确认家庭信息；
2. CHFH 判断现金流、债务、保障和责任是否安全；
3. 动态四账户与 ELTC Bridge 解释为什么只有部分金融资源适合长期投资；
4. GRB 分别展示风险承担能力、风险意愿、行为约束和最终家庭风险预算；
5. 组合引擎只配置通过资金资格闸门的 ELTC；
...
- 家庭、Golden Outcomes、benchmark 与主 Demo 使用合成数据，不代表真实工行客户或中国家庭总体分布。
- 公开基金资料是带核验日期的 public snapshot，不是实时工行产品货架；官网公开列示也不等于任何客户当日可申购。
- 银行 IAM、客户、AML/CDD、产品、交易、CRM 与投研接口仍为 Mock 或 fail-closed adapter，没有 Live ICBC integration。
- 系统不开户、不交易、不自动调仓，不承诺本金、收益、目标达成率或经营效果。
- 当前效果验证以工程回归和合成家庭对照为主，不构成真实 AUM、转化率、顾问效率或投诉改善证据。

完整限制见 [`docs/known_limitations.md`](docs/known_limitations.md)。
